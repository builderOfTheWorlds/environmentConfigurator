#!/usr/bin/env python3
"""
Daily Conversation History Analyzer for Claude Code
...
class ComprehensiveAnalyzer:
    def __init__(self, claude_dir: str = "~/.claude"):
        ...
        self.main_history = self.claude_dir / "history.jsonl"
        self.projects_dir = self.claude_dir / "projects"
        self.log_dir = self.claude_dir / "analysis_logs"
    ...
    def load_all_conversations(self) -> Tuple[List[Dict], Dict[str, int]]:
        ...
        if self.projects_dir.exists():
            ...
            for project_file in project_files:
                # Skip agent files for now (they're subprocesses, not user conversations)
                if 'agent-' in project_file.name:
                    sources['agent_files'] += 1
                    continue
            ...
            self.log(f"Skipped {sources['agent_files']} agent subprocess files")
    ...
    def generate_report(self) -> str:
        ...
        report = []
        report.append("=" * 100)
        report.append("CLAUDE CODE COMPREHENSIVE CONVERSATION ANALYSIS")
        report.append("=" * 100)
    ...
    def run(self):
        ...
        try:
            ...
            latest_report = self.claude_dir / "latest_analysis.txt"
