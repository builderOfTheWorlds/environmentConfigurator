...
LOG_FILE="$HOME/.gemini/analysis_logs/cron_$(date +%Y%m%d_%H%M%S).log"

# Create log directory if it doesn't exist
mkdir -p "$HOME/.gemini/analysis_logs"
...
HISTORY_FILE="$HOME/.gemini/history.jsonl"

if [ ! -f "$HISTORY_FILE" ]; then
    log "WARNING: History file not found at $HISTORY_FILE"
    log "Running analysis anyway..."
    python3 "$HOME/.gemini/daily_analysis.py" 2>&1 | tee -a "$LOG_FILE"
    exit $?
fi
